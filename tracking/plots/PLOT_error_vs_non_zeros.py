...
ax1 = plt.subplot(211)
ax1.semilogx(x[START_PLOT:END_PLOT],bin_counts[START_PLOT:END_PLOT], color='tab:orange')
ax1.set_xlim([START_PLOT+1,END_PLOT+1])
ax1.set_ylim([0,np.max(bin_counts[START_PLOT:END_PLOT])])
# ax1.set_ylim([0,4000])
...
plt.subplot(212)
plt.semilogx(x[START_PLOT:END_PLOT], non_zero_bins_mean[START_PLOT:END_PLOT], label='Average Error')
plt.fill_between(x[START_PLOT:END_PLOT], non_zero_bins_bot[START_PLOT:END_PLOT],
    non_zero_bins_top[START_PLOT:END_PLOT], facecolor='tab:blue', alpha=0.3,
    label = '$95\%$ Interval')
plt.legend()
plt.xlim([START_PLOT+1,END_PLOT+1])
# plt.ylim([0,min(np.max(non_zero_bins_top), 50)])
plt.ylim([0,min(np.max(non_zero_bins_top[START_PLOT:END_PLOT]), 20)])
plt.ylabel("Error (m)")
plt.xlabel("Number of Detected Paths (log scale)")
plt.grid(True)
